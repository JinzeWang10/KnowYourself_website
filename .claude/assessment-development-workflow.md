...
3. 添加所有题目
   - 确保每个题目有唯一ID
   - **⭐ 重要**：题目类型必须根据量表性质选择正确的 `type`：
     - **Likert 量表**（如 1-5 分制、1-7 分制）→ 使用 **`type: 'likert'`**（横向选项栏）
     - 单选题 → 使用 `type: 'single'`
     - 多选题 → 使用 `type: 'multiple'`
     - ❌ **不要使用** `type: 'scale'`（旧的通用类型，会导致 UI 展示问题）
   - 正确设置 `dimension` 字段
   - 反向题标记 `reversed: true`
   - 选项值必须是数字 (1-5 或 1-7)
   - **🎨 选项顺序规范（极其重要！）**：
     - Likert 横向选项栏有视觉语义（交通灯原则）：**左侧（绿色）=低分/健康，右侧（红色）=高分/需关注**
     - **必须按照 value 从小到大排列**：`{ value: 1, label: '没有' }` 在前，`{ value: 5, label: '严重' }` 在后
     - ✅ 正确：`[{value: 1, label: '没有'}, ... , {value: 5, label: '严重'}]` → 左绿（健康）右红（症状）
     - ❌ 错误：`[{value: 5, label: '严重'}, ... , {value: 1, label: '没有'}]` → 反了！
     - 验证：第一项=最小value+消极语义（没有/不同意/从未），最后一项=最大value+积极语义（严重/同意/总是）
   - 📌 **参考**：[lib/scales/eq.ts](lib/scales/eq.ts) 和 [lib/scales/scl90.ts](lib/scales/scl90.ts) 正确使用了 `type: 'likert'` 和正确的选项顺序
...
|-----------|--------------|---------|----------|
| 1-5 分制 | 5 | 直接使用因子分/维度分 | SCL-90, ESS, INI |
| 1-7 分制 | 7 | 直接使用因子分/维度分 | 某些自评量表 |
| 0-100 百分比 | 100 | 使用归一化分数 | EQ, PAT, Workhorse |
| 0-1 归一化 | 100 | 乘以 100 转为百分比 | ZHZ, Zootopia |

**实现位置**: 在 ShareCard 渲染逻辑中（约 line 258）
...
// 在 (() => { ... })() 的雷达图数据准备部分添加
  ...
  radarData = scale.dimensions?.map((dimension) => ({
    dimension: dimension.name,
    value: Math.round((result.dimensionScores![dimension.id] || 0) * 100 * 10) / 10,
    fullMark: 100,
  }));
} else {
  // 其他量表使用标准归一化
...
**如何选择处理方式**:
1. 查看量表的 `scoring.scaleRange` 和维度计分方式
2. 参考同类评分制的现有量表
3. 如果因子分/维度分是原始分数（1-5, 1-7），使用方式 1
4. 如果已经归一化为百分比（0-100），使用方式 2
5. 如果是 0-1 的相似度分数，使用方式 3

**调试技巧**:
```typescript
// 临时添加 console.log 查看雷达图数据
console.log('雷达图数据:', radarData);
// 检查: value 是否在合理范围？fullMark 是否正确？
```
...
**检查清单**:
- [ ] 结果数据正确读取
- [ ] **雷达图 fullMark 与评分制匹配** ⚠️
- [ ] **雷达图数值在合理范围内（不全是 0 或 100）** ⚠️
- [ ] UI/UX 与其他测评一致
- [ ] 响应式设计 (移动端适配)
- [ ] 分享功能正常
- [ ] 返回首页链接正常
...
- [ ] **维度雷达图正确 (如适用)** ⚠️ 重点检查:
  ...
  - [ ] fullMark 与量表评分制匹配（1-5 分制 → fullMark=5, 0-100 → fullMark=100）
  - [ ] 雷达图形状能反映真实的维度差异（高低分可视化）
...
| 量表 | 类型 | 计分方式 | **评分制** | **雷达图 fullMark** | 特殊功能 | 参考实现 |
|------|------|---------|-----------|-------------------|---------|---------|
| ESS | 简单 | 求和 | 1-5 分制 | **5** | 反向题 | 模式1 |
| INI | 简单 | 求和 | 1-5 分制 | **5** | 多分数段 | 模式1 |
| BES | 简单 | 求和 | 1-5 分制 | **5** | - | 模式1 |
| **SCL-90** | **复杂** | **自定义** | **1-5 分制因子分** | **5** | **危机预警** | **模式2 + 独立文件** |
| ANI | 复杂 | 自定义 | 百分比 | 100 | 多维度复杂计分 | 模式2 + 独立文件 |
| PAT | 复杂 | 自定义 | 0-100 | 100 | 心理年龄计算 | 模式2 + metadata |
| ZHZ | 复杂 | 自定义 | 0-1 归一化 | 100 | 角色匹配 | 模式2 + 自定义结果页 |
| Zootopia | 复杂 | 自定义 | 0-1 归一化 | 100 | 角色匹配 | 模式2 + 自定义结果页 |
| Workhorse | 复杂 | 自定义 | 0-100 | 100 | 维度解读 | 模式2 |
| EQ | 混合 | 自定义 | 0-100 | 100 | 正反向题 | 模式3 |
...
// 结果: 雷达图显示 1.47/100 ≈ 1.5%，视觉上几乎看不见
```
...
// 结果: 雷达图显示 1.47/5 ≈ 29%，清晰可见
```

**如何避免**:
1. 在开发前明确量表的评分制（见步骤 5.1）
2. 参考同类评分制的现有量表（如 ESS 也是 1-5 分制）
3. 测试时检查雷达图数值范围是否合理
4. 使用浏览器控制台打印 `radarData` 检查数据
