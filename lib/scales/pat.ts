...
export const pat: QuizTemplate = {
  ...
  questions: [
    // ========== 维度A：情绪成熟度（Emotional Maturity）7题 ==========
    {
      ...
      options: [
        { value: 0, label: '完全不符合' },
        { value: 0.625, label: '较不符合' },
        { value: 1.25, label: '一般' },
        { value: 1.875, label: '较符合' },
 ...
 * @param actualAge 用户实际年龄
 * @returns 心理年龄
 */
export function calculatePsychologicalAge(score: number, actualAge: number): number {
  // 根据分数段计算心理年龄偏移量
  let ageOffset = 0;

  if (score >= 81) {
    // 极高成熟度：心理年龄 = 实际年龄 + 10岁以上
    // 线性映射：81分对应+10岁，100分对应+15岁
    ageOffset = 10 + ((score - 81) / 19) * 5;
  } else if (score >= 63) {
    // 高成熟度：心理年龄 = 实际年龄 + 5~10岁
    // 线性映射：63分对应+5岁，80分对应+10岁
    ageOffset = 5 + ((score - 63) / 17) * 5;
  } else if (score >= 44) {
    // 中等成熟度：心理年龄 = 实际年龄 ±3岁
    // 线性映射：44分对应-3岁，53分对应0岁，62分对应+3岁
    const midPoint = 53; // 中等成熟度的中点
    if (score >= midPoint) {
      ageOffset = ((score - midPoint) / (62 - midPoint)) * 3;
    } else {
      ageOffset = -3 + ((score - 44) / (midPoint - 44)) * 3;
    }
  } else if (score >= 25) {
    // 发展中阶段：心理年龄 = 实际年龄 - 5~10岁
    // 线性映射：25分对应-10岁，43分对应-5岁
    ageOffset = -10 + ((score - 25) / 18) * 5;
  } else {
    // 心理发展滞后：心理年龄 = 实际年龄 - 10岁以上
    // 线性映射：0分对应-15岁，24分对应-10岁
    ageOffset = -15 + (score / 24) * 5;
  }

  // 计算心理年龄并确保合理范围
  const psychologicalAge = Math.round(actualAge + ageOffset);

  // 心理年龄不应低于8岁（基本认知能力形成的年龄）
  // 心理年龄上限为实际年龄+20岁（避免过度夸张）
  return Math.max(8, Math.min(psychologicalAge, actualAge + 20));
}
