...
export const workhorse: QuizTemplate = {
  ...
  category: '职场评估',
  purpose: '从工作负荷、薪酬与福利不足、制度与氛围问题、发展空间受限程度和个人心理疲惫感五个维度，全面评估你的工作是否"牛马化"。通过科学的加权计算和多维度分析，帮助你识别工作中的不合理状况，为职业决策提供参考。',
  duration: '6-10分钟',
  questionCount: 20,
  ...
  scoring: {
    ...
    ranges: [
      ...
      },
      {
        ...
        color: '#f97316',
        title: '中度牛马化',
        description: '你的工作在某些方面存在较为突出的问题，整体处于中度压力状态。你像是不小心进了"高压锅模式"却一直没找到暂停键。你仍然坚韧、有能力，也努力，但这份投入换来的幸福值有点不划算。建议查看下方的综合分析和维度详情，了解具体需要改善的方面。',
        suggestions: [
          '强烈建议评估当前工作的可持续性',
          '考虑主动寻找新的工作机会',
          '优先关注身心健康，避免过劳',
      ...
      },
      {
        ...
        color: '#ef4444',
        title: '高度牛马化',
        description: '你的工作环境属于：只差马鞍和草料。你的得分已经呈现典型"牛马化"特征。你可能常常觉得"做多少都不够"，你的身心能量一直在被快速消耗。看到这个分数，一般人可能会沉默两秒，然后开始劝你："要不……先考虑下自己？" 如果你正在寻找改变，这个结果是一次非常明确的提醒。请查看下方的综合分析和维度详情，了解具体的问题所在。',
        suggestions: [
          '立即关注身心健康状况',
          '强烈建议开始寻找新工作',
          '考虑休假或暂时离职调整状态',
  ...
  calculateResults: (answers: Record<string, number>) => {
    // 维度配置
    const dimensions = [
      { id: 'workload', questionIds: ['wh_1', 'wh_2', 'wh_3', 'wh_4'], maxScore: 65, weight: 0.35, name: '工作负荷' },
      { id: 'benefits', questionIds: ['wh_5', 'wh_6', 'wh_7', 'wh_8', 'wh_9'], maxScore: 54, weight: 0.20, name: '薪酬与福利不足' },
      { id: 'environment', questionIds: ['wh_10', 'wh_11', 'wh_12', 'wh_13'], maxScore: 42, weight: 0.15, name: '制度与氛围问题' },
      { id: 'growth', questionIds: ['wh_14', 'wh_15', 'wh_16'], maxScore: 30, weight: 0.10, name: '发展空间受限程度' },
      { id: 'mental', questionIds: ['wh_17', 'wh_18', 'wh_19', 'wh_20'], maxScore: 39, weight: 0.20, name: '个人心理疲惫感' },
    ];

    // 计算各维度得分和原始分数
    const dimensionScores: Record<string, number> = {}; // 100分制的维度得分
    ...
    dimensions.forEach(dimension => {
      ...
      // 维度得分（100分制）= (用户得分 / 该维度满分) × 100
      const dimensionScore = (rawScore / dimension.maxScore) * 100;
    ...
    // 根据维度等级生成描述性短语
    const getLevelDescription = (dimId: string, level: string): string => {
      const dimName = dimensionDisplayNames[dimId];

      if (dimId === 'workload') {
        if (level === '轻松') return `**${dimName}**较轻，工作时间合理、通勤便利`;
        if (level === '适中') return `**${dimName}**适中，压力可控`;
        if (level === '较重') return `**${dimName}**偏重，工作时长和通勤压力较大`;
        return `**${dimName}**极重，长期处于高强度工作状态`;
      }

      if (dimId === 'benefits') {
        if (level === '优秀') return `**${dimName}**优秀，收入满意度高、保障完善`;
        if (level === '良好') return `**${dimName}**良好，基本满足生活需求`;
        if (level === '一般') return `**${dimName}**一般，收入不够理想`;
        return `**${dimName}**较差，经济压力大、缺乏保障`;
      }

      if (dimId === 'environment') {
        if (level === '健康') return `**${dimName}**健康，团队氛围轻松、制度透明`;
        if (level === '普通') return `**${dimName}**普通，整体可接受`;
        if (level === '压抑') return `**${dimName}**压抑，存在内卷或不公平现象`;
        return `**${dimName}**恶劣，充满敌意和压迫`;
      }

      if (dimId === 'growth') {
        if (level === '广阔') return `**${dimName}**广阔，有清晰的晋升通道和成长机会`;
        if (level === '有限') return `**${dimName}**有限，发展机会不够充分`;
        if (level === '狭窄') return `**${dimName}**狭窄，晋升困难`;
        return `**${dimName}**停滞，几乎无晋升可能`;
      }

      if (dimId === 'mental') {
        if (level === '良好') return `**${dimName}**良好，对工作保持积极态度`;
        if (level === '轻度疲惫') return `**${dimName}**轻度疲惫，偶有倦怠感`;
        if (level === '中度倦怠') return `**${dimName}**中度倦怠，经常感到被压榨`;
        return `**${dimName}**严重倦怠，每天都想逃离`;
      }

      return `**${dimName}**${level}`;
    };

    // 根据总分区间生成不同的简评结构
    if (totalScore <= 20) {
      // 完全不牛马：展示所有维度都很好
      interpretation = `你的工作整体状态良好。${getLevelDescription(bestDimensions[0].id, bestDimensions[0].level)}，${getLevelDescription(bestDimensions[1].id, bestDimensions[1].level)}，展现出健康的职场生态。继续保持这种平衡，你像是职场里的"自由灵魂"。`;
    } else if (totalScore <= 40) {
      // 略带牛马影子：有好有坏，强调问题但还可控
      const worstDesc = getLevelDescription(worstDimensions[0].id, worstDimensions[0].level);
      const bestDesc = getLevelDescription(bestDimensions[0].id, bestDimensions[0].level);
      interpretation = `你的工作状态有些许压力但整体可控。${worstDesc}，但${bestDesc}。注意节奏调整，避免长期处于"半牛马"状态。`;
    } else if (totalScore <= 60) {
      // 轻度牛马化：明显不平衡，强调问题维度
      const worst1 = getLevelDescription(worstDimensions[0].id, worstDimensions[0].level);
      const worst2 = getLevelDescription(worstDimensions[1].id, worstDimensions[1].level);
      const best1 = getLevelDescription(bestDimensions[0].id, bestDimensions[0].level);
      interpretation = `你的工作存在明显的不平衡。${worst1}，${worst2}，给你带来较大压力。虽然${best1}，但整体付出与回报不太匹配，需要认真评估是否值得长期坚持。`;
    } else if (totalScore <= 80) {
      // 中度牛马化：多个维度有问题，状态令人担忧
      const worst1 = getLevelDescription(worstDimensions[0].id, worstDimensions[0].level);
      const worst2 = getLevelDescription(worstDimensions[1].id, worstDimensions[1].level);
      interpretation = `你的工作状态令人担忧。${worst1}，${worst2}，已经处于典型的被动付出状态。这种状态往往难以持续，建议尽快寻找改善途径。`;
    } else {
      // 高度牛马化：极度恶劣，强烈警告
      const worst1 = getLevelDescription(worstDimensions[0].id, worstDimensions[0].level);
      const worst2 = getLevelDescription(worstDimensions[1].id, worstDimensions[1].level);
      interpretation = `你的工作状态已达到危险水平。${worst1}，${worst2}，你几乎在为公司燃烧生命。请立即关注身心健康，必要时考虑改变方向。`;
    }
